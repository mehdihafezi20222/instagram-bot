...
CREDIT_MESSAGE = "🙏 با تشکر از امپراطور فانی"

# فقط اینستاگرام و تیک‌تاک پشتیبانی می‌شوند
SUPPORTED_DOMAINS = [
...
# ========== تشخیص عکس (رفع باگ کوتیشن که باعث کرش می‌شد) ==========
...
async def fetch_preview_info(url: str):
    ...
    def _probe():
        opts = {
            ...
            "ignoreerrors": True,
            "skip_download": True,
            # برای اینستاگرام: اگر لینک آلبوم/img_index نداشته باشد noplaylist فعال است
            "noplaylist": (not is_instagram) or ("img_index=" in url),
            "user_agent": (
                "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
                "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
            ),
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "لینک پست اینستاگرام یا تیک‌تاک را بفرست تا گزینه‌های دانلود را نشان بدهم.",
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "راهنما:\n"
        "• لینک اینستاگرام یا تیک‌تاک بفرست\n"
        "• می‌توانی چند لینک را در یک پیام بفرستی\n"
        "• از منو برای پروفایل و آمار استفاده کن\n"
    ...
    if not urls:
        if update.message.text:
            await update.message.reply_text(
                ...
                "سایت‌های پشتیبانی‌شده: اینستاگرام، تیک‌تاک"
            )
        return
...
async def process_single_url(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str):
    ...
    if "/stories/" in urlparse(url).path.lower() and not has_cookies:
        await update.message.reply_text(
            ...
            "این محدودیت خود اینستاگرام است، نه مشکل ربات. لینک‌های پست و ریلز عادی کار می‌کنند."
        )
        return
    ...
    # برای ویدیوها منوی کیفیت نمایش داده شود
    heights = []
    if preview_info:
...
async def _download_and_send_real(status_msg, user, url, quality, job_id, short_id, job_dir, preview_info=None):
    ...
    ydl_opts = {
        ...
        "no_warnings": True,
        "ignoreerrors": True,
        # برای عکس (آلبوم) noplaylist را False می‌کنیم تا همه آیتم‌های آلبوم دانلود شوند
        "noplaylist": (not is_image) and ((not is_instagram) or ("img_index=" in url)),
        "user_agent": (
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
            "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
        ),
...
async def menu_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📥 دانلود":
        await update.message.reply_text("لینک پست اینستاگرام یا تیک‌تاک را بفرست تا گزینه‌های دانلود را نشان بدهم.")
